...
description: >-
  Design and present the simulation/numerical-results section of an IEEE communications paper
  (JSAC, TWC, TC/TCOM, WCL, CL) so the evidence matches the claims — choosing benchmark schemes,
  Monte-Carlo protocol, communications metrics (BER/SER, outage, ergodic/sum rate, spectral and
  energy efficiency, coverage), parameter sweeps over SNR / antennas / users, analysis-vs-simulation
  validation, and convergence/complexity comparison. Also covers learning-based comms evaluation
  (NMSE, model-based vs learned benchmarks, generalization, inference cost) and ISAC dual-metric
  evaluation (CRB/detection plus the rate–CRB tradeoff region). Use this whenever the user is
  planning or auditing a results section: "what
  benchmark schemes do I need", "is my simulation convincing", "design the numerical results", "my
  curves don't support the claim", "how to evaluate my neural network", "对比方案怎么选", "蒙特卡洛仿真",
  "仿真和理论对不上", "审稿人会质疑我的仿真吗", "神经网络怎么对比", "感知通信一体化", "CRB", "性能折中曲线". For
  writing the results *narrative*, hand to ieee-writing; for plotting, hand to ieee-figure.
version: 0.4.0
author: WT — based on IEEE communications evaluation practice (Monte-Carlo simulation, analysis validation, benchmark schemes)
---
