...
A bilingual (中文 / English) collection of Codex skills for writing **IEEE communications-journal**
...
This folder is a local Codex plugin. Its manifest is `.codex-plugin/plugin.json`, and its skills are
under `skills/`.

Install it through your Codex plugin workflow from the folder path, or add it to a local marketplace
if you maintain one.

### Direct Codex skills

Copy any `skills/ieee-*/` directory (with its `references/` and `agents/`) into your Codex skills
folder, then restart Codex:

```bash
cp -R skills/ieee-writing ~/.codex/skills/
cp -R skills/ieee-polishing ~/.codex/skills/
```
...
1. Create `skills/ieee-<topic>/` with `SKILL.md` (frontmatter `name` + `description`) and
   `agents/openai.yaml`; add `references/*.md` for anything that would push `SKILL.md` past ~400
   lines.
